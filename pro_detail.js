...
btnAjout.addEventListener('click',function(){ 
  let btnid=btnAjout.dataset.id
 /** let value = parseInt(btnQ.textContent);**/
  const produits = new Produits();
  const ui= new AjoutProd ();

  produits
    .AccProduits()
   .then((produit)=>{/*produit.forEach((prod)=>{prod.pro_quant = 1;})*/
    ui.ajoutCart(produit,value,btnid);
